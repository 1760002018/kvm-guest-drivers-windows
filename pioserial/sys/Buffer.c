...
static BOOLEAN DmaWriteCallback(PPHYZIO_DMA_TRANSACTION_PARAMS params);
...
)
{
    ...
    buf->va_buf = PhyzIOWdfDeviceAllocDmaMemory(vdev, buf_size, id);
    if(buf->va_buf == NULL)
    {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_QUEUEING, "PhyzIOWdfDeviceAllocDmaMemory failed, %s::%d\n", __FUNCTION__, __LINE__);
        ExFreePoolWithTag(buf, PIOSERIAL_DRIVER_MEMORY_TAG);
        return NULL;
    }
    buf->pa_buf = PhyzIOWdfDeviceGetPhysicalAddress(vdev, buf->va_buf);
    if (!buf->pa_buf.QuadPart) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_QUEUEING, "PhyzIOWdfDeviceGetPhysicalAddress failed, %s::%d\n", __FUNCTION__, __LINE__);
        PhyzIOWdfDeviceFreeDmaMemory(vdev, buf->va_buf);
        ExFreePoolWithTag(buf, PIOSERIAL_DRIVER_MEMORY_TAG);
        return NULL;
    }
...
size_t PIOSerialSendBuffers(IN PPIOSERIAL_PORT Port,
                            IN PWRITE_BUFFER_ENTRY Entry)
{
    struct virtqueue *vq = GetOutQueue(Port);
    PHYZIO_DMA_TRANSACTION_PARAMS params;
    RtlZeroMemory(&params, sizeof(params));
    params.allocationTag = PIOSERIAL_DRIVER_MEMORY_TAG;
    params.param1 = Port;
    params.param2 = Entry;
    params.buffer = Entry->OriginalWriteBuffer;
    params.size = (ULONG)Entry->OriginalWriteBufferSize;
    return !!PhyzIOWdfDeviceDmaTxAsync(vq->vdev, &params, DmaWriteCallback);
}

BOOLEAN
DmaWriteCallback(PPHYZIO_DMA_TRANSACTION_PARAMS params)
{
    PPIOSERIAL_PORT Port = params->param1;
    NTSTATUS status = STATUS_INSUFFICIENT_RESOURCES;
    PWRITE_BUFFER_ENTRY Entry = params->param2;
    struct virtqueue *vq = GetOutQueue(Port);
    struct PhyzIOBufferDescriptor sg[QUEUE_DESCRIPTORS];
    int prepared = 0, ret;
    ULONG i = 0;
    ...
    status = WdfRequestMarkCancelableEx(Entry->Request,
        PIOSerialPortWriteRequestCancel);
...
error:
    PhyzIOWdfDeviceDmaTxComplete(vq->vdev, params->transaction);
...
)
{
    ...
    if (buf->va_buf)
    {
        PhyzIOWdfDeviceFreeDmaMemory(buf->vdev, buf->va_buf);
        buf->va_buf = NULL;
    }
    ExFreePoolWithTag(buf, PIOSERIAL_DRIVER_MEMORY_TAG);
    TraceEvents(TRACE_LEVEL_VERBOSE, DBG_QUEUEING, "<-- %s\n", __FUNCTION__);
...
VOID PIOSerialProcessInputBuffers(IN PPIOSERIAL_PORT Port)
{
    NTSTATUS status;
    ULONG Read;
    ...
    if (Port->InBuf && Port->PendingReadRequest)
    {
        ...
        if (status != STATUS_CANCELLED)
        {
            ...
            if (NT_SUCCESS(status))
            {
                ...
                Read = (ULONG)PIOSerialFillReadBufLocked(Port, Buffer, Length);
            }
            else
            {
...
BOOLEAN PIOSerialReclaimConsumedBuffers(IN PPIOSERIAL_PORT Port)
{
    WDFREQUEST request;
    ...
    while ((iter = PopEntryList(&ReclaimedList)) != NULL)
    {
        ...
        if (entry->dmaTransaction) {
            PhyzIOWdfDeviceDmaTxComplete(vq->vdev, entry->dmaTransaction);
        }
        request = entry->Request;
        if (request != NULL)
        {
...
)
{
    ...
    if (!count || !PIOSerialPortHasDataLocked(port))
        return 0;
    ...
    if (buf->offset == buf->len)
    {
        ...
        status = PIOSerialAddInBuf(GetInQueue(port), buf);
        if (!NT_SUCCESS(status))
        {
           TraceEvents(TRACE_LEVEL_ERROR, DBG_QUEUEING, "%s::%d  PIOSerialAddInBuf failed\n", __FUNCTION__, __LINE__);
        }
    }
    TraceEvents(TRACE_LEVEL_VERBOSE, DBG_QUEUEING, "<-- %s\n", __FUNCTION__);
    return count;
