...
{
    wstring name;
    for (int i = 0; FindInstance(GUID_PIOSERIAL_CONTROLLER, i, name); ++i)
    {
...
{
    wstring name;
    for (int i = 0; FindInstance(GUID_PIOSERIAL_PORT, i, name); ++i)
    {
        AddPort(name.c_str());
    }
}
...
void PnPControl::Run()
{
    ...
    wc.lpszClassName = L"PioSerialLib";
    ...
    ControllerNotify = RegisterInterfaceNotify(GUID_PIOSERIAL_CONTROLLER);
    PortNotify = RegisterInterfaceNotify(GUID_PIOSERIAL_PORT);
...
HDEVNOTIFY PnPControl::RegisterHandleNotify(HANDLE handle)
{
    ...
    NotificationFilter.dbch_eventguid = GUID_PIOSERIAL_PORT_CHANGE_STATUS;
...
void PnPControl::ProcessPnPNotification(PnPControl* ptr, PnPNotification Notification)
{
    ...
    case DBT_DEVICEARRIVAL: {
        ...
        if (pHdr->dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE) {
            ...
            if (IsEqualGUID(GUID_PIOSERIAL_CONTROLLER, pDevInf->dbcc_classguid))
            {
                ptr->AddController(pDevInf->dbcc_name);
            }
            else if (IsEqualGUID(GUID_PIOSERIAL_PORT, pDevInf->dbcc_classguid))
            {
...
VOID PnPControl::RegisterNotification(PVOID port, PIOSERIALNOTIFYCALLBACK pfn, PVOID ptr)
{
    for(Iterator it = Ports.begin(); it != Ports.end(); it++)
    {
        if (*it == port)
