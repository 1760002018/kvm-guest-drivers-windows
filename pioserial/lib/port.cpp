...
SerialPort::SerialPort(wstring LinkName, PnPControl* ptr)
{
    ...
    BYTE buf[512];
    PPHYZIO_PORT_INFO inf = (PPHYZIO_PORT_INFO)buf;
...
void SerialPort::handleEvent(const PnPControl& ref)
{
    ...
    case DBT_CUSTOMEVENT: {
        ...
        if (IsEqualGUID(GUID_PIOSERIAL_PORT_CHANGE_STATUS, pHdr->dbch_eventguid) &&
            (pHdr->dbch_handle == Handle))
        {
            PPHYZIO_PORT_STATUS_CHANGE pEventInfo = (PPHYZIO_PORT_STATUS_CHANGE) pHdr->dbch_data;
