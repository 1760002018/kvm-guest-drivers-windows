...
BOOL CDevice::Init(BOOL ovrl, UINT index)
{
    ...
    if ((DevicePath = GetDevicePath(index, (LPGUID)&GUID_PIOSERIAL_PORT)) != NULL)
    {
        ...
        if (m_hDevice != INVALID_HANDLE_VALUE)
        {
            printf("Open pioserial device  %S.\n", DevicePath);
            return TRUE;
        }

    }
    DWORD err = GetLastError();
    printf("Cannot find pioserial device. %S , error = %d\n", DevicePath, err );
    return FALSE;
}
...
BOOL CDevice::Write(PVOID buf, size_t *size)
{
    ...
    if (!res)
    {
        printf("Cannot write pioserial device.\n");
    }
    else if ( ret != bytes)
    {
        printf("Write pioserial device error. written = 0x%x, expected = 0x%x\n", ret, bytes);
        *size = ret;
        ret = FALSE;
    }
    return res;
...
BOOL CDevice::Read(PVOID buf, size_t *size)
{
    ...
    {
        printf("Read pioserial device error. get = 0x%x, expected = 0x%x\n", ret, bytes);
        *size = ret;
        ret = FALSE;
    }
