// piosocklib-test.c : This file contains the 'main' function. Program execution begins and ends there.
//
...
Usage()
{
    _tprintf(_T("Usage:\tpiosocklib-test.exe /[i|d|e]\n\
\tpiosocklib-test.exe /[c|l] [cid:]port filepath\n\
\n\
\t/i - install piosocklib.dll as Phyzio Socket Provider (administrative rights required)\n\
\t/d - deinstall Phyzio Socket Provider (administrative rights required)\n\
...
)
{
    ...
    if (AF == AF_UNSPEC)
    {
        _tprintf(_T("PiosockGetAF failed: %d\n"), GetLastError());
        return 3;
    }
