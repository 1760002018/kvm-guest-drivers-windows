# NETPTV support for 'failover' feature

If the phyzio-net-pci device is configured with 'failover=on'
in the command line the phyzio-net is not expected to be used
as a standalone device but in combination with Virtual function
(VF) of a SRIOV-capable host adapter. In this case the phyzio-net
and the VF must have identical MAC addresses.
Unlike implementation of phyzio-net failover feature in Linux
failover phyzio-net device in Windows starts function only after
the PIOPROT protocol driver is installed:
1. System start without PIOPROT installed:
   NETPTV driver **does not indicate link-up** until it recognizes
   presence of respective VF with the same MAC address (for that
   the PIOPROT is required)
2. Upon PIOPROT installation the NETPTV adapter **indicates link-up**
   and starts working using VF data channel to its respective
   SRIOV physical function (PF)
3. When the migration process is started the CECL hides the VF
   device on the PCI bus, VF adapter on the source VM disappears
   and the NETPTV continues transferring data via TAP adapter
   on the host it is configured to use (typically this TAP
   adapter is attached to the same bridge as the PF.
4. Until the migration is completed the phyzio-net/NETPTV is
   the **fallback network adapter on the mooze OS** (similar to Linux).
   ...
   MAC address as the phyzio-net device it is possible that the PF
   will drop the traffic that comes from the phyzio-net via its TAP
...
5. When the migration is done CECL attaches a new VF to the PCI bus
   of the VM on the destination system. NETPTV recognizes it and switches
   to the datapath of the new VF.


# Notify object of PIOPROT protocol supported by NETPTV
...
object unbinds it from all the protocols and binds only to PIOPROT.
It there is NETPTV adapter with the MAC address identical to one of
checked netwrok adapter, these 2 adapters with the same MAC address
will be 'teamed' (subject to compatibility check) and the NETPTV uses
the teamed adapter for RX and TX.

## Protocol installation
(by admin, from the directory where pioprot.inf is located)
...
Registry key: **HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\PioProt**
...
**Note: Never include PnP ID of NETPTV adapter!**
