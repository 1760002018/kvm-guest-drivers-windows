...
pRxNetDescriptor CParaNdisRX::CreateRxDescriptorOnInit()
{
    ...
    //  1 page for phyzio header and indirect buffers array
    //  X pages needed to fit maximal length buffer of data
    //  The assumption is phyzio header and indirect buffers array fit 1 page
    ...
    p->BufferSGArray = (struct PhyzIOBufferDescriptor *)
    ...
    //First page is for phyzio header, size needs to be adjusted correspondingly
    p->BufferSGArray[0].length = m_Context->nPhyzioHeaderSize;

    ULONG indirectAreaOffset = ALIGN_UP(m_Context->nPhyzioHeaderSize, ULONGLONG);
...
VOID CParaNdisRX::ProcessRxRing(CCHAR nCurrCpuReceiveQueue)
{
    ...
    while (NULL != (pBufferDescriptor = (pRxNetDescriptor)m_VirtQueue.GetBuf(&nFullLength)))
    {
        ...
        packetAnalysisRC = ParaNdis_PerformPacketAnalysis(
            ...
            nFullLength - m_Context->nPhyzioHeaderSize);
