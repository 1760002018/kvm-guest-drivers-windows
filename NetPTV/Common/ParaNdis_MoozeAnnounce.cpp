...
#include "ParaNdis_MoozeAnnounce.h"
#include "ParaNdis6_Driver.h"
#include "ethernetutils.h"
#include "Trace.h"
#ifdef NETPTV_WPP_ENABLED
#include "ParaNdis_MoozeAnnounce.tmh"
#endif

CMoozeAnnouncePacketHolder::~CMoozeAnnouncePacketHolder()
{
    PVOID buffer;
    PMDL mdl;
    if (m_NBL)
...
void CMoozeAnnouncePacketHolder::OnLastReferenceGone()
{
    Destroy(this, m_handle);
}

EthernetArpFrame *CMoozeAnnouncePackets::CreateIPv4Packet(UINT32 IPV4)
{
...
EthernetNSMFrame *CMoozeAnnouncePackets::CreateIPv6Packet(USHORT * IPV6)
{
...
VOID CMoozeAnnouncePackets::CreateNBL(PVOID packet, UINT size, bool isIPV4)
{
    ...
    CMoozeAnnouncePacketHolder *PacketHolder = new (m_Context->MiniportHandle) CMoozeAnnouncePacketHolder(nbl, m_Context->MiniportHandle, isIPV4);
    if (!PacketHolder)
    {
...
VOID CMoozeAnnouncePackets::CreateNBL(UINT32 IPV4)
{
...
VOID CMoozeAnnouncePackets::CreateNBL(USHORT *IPV6)
{
...
VOID CMoozeAnnouncePackets::SendNBLs()
{
    m_packets.ForEach([this](CMoozeAnnouncePacketHolder* GratARPPacket)
    {
...
void CMoozeAnnouncePackets::NblCompletionCallback(PNET_BUFFER_LIST NBL)
{
    CMoozeAnnouncePacketHolder *GratARPPacket = (CMoozeAnnouncePacketHolder *)NBL->MiniportReserved[0];
    if (GratARPPacket)
    {
        auto OriginalNBL = GratARPPacket->GetNBL();
        NETPTV_ASSERT(NBL->ParentNetBufferList == OriginalNBL);
