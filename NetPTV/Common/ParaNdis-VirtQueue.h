...
public:
    ...
    bool Create(NDIS_HANDLE DrvHandle, ULONG PhyzioHdrSize) 
    {
        m_PhyzioHdrSize = PhyzioHdrSize;
        return m_HeadersBuffer.Create(DrvHandle);
    }

    bool Allocate();

    phyzio_net_hdr *PhyzioHeader() const
    { return static_cast<phyzio_net_hdr*>(m_PhyzioHeaderVA); }
    ULONG PhyzioHeaderLength() const
    { return m_PhyzioHdrSize; }
    ...
    PHYSICAL_ADDRESS PhyzioHeaderPA() const
    { return m_PhyzioHeaderPA; }
...
public:
    ...
    bool Create(NDIS_HANDLE DrvHandle,
                  ULONG PhyzioHeaderSize,
                  struct PhyzIOBufferDescriptor *PhyzioSGL,
                  ULONG PhyzioSGLSize,
                  bool Indirect,
                  bool AnyLayout)
    {
        if (!m_Headers.Create(DrvHandle, PhyzioHeaderSize))
            return false;
        if (!m_IndirectArea.Create(DrvHandle))
            return false;
        m_PhyzioSGL = PhyzioSGL;
        m_PhyzioSGLSize = PhyzioSGLSize;
        m_Indirect = Indirect;
...
private:
    ...
    struct PhyzIOBufferDescriptor *m_PhyzioSGL;
    ULONG m_PhyzioSGLSize;
    ULONG m_CurrPhyzioSGLEntry;
...
public:
    ...
    ULONG GetRingSize()
    { return phyzio_get_queue_size(m_VirtQueue); }
    ...
    int AddBuf(struct PhyzIOBufferDescriptor sg[],
...
private:
    ...
    struct PhyzIOBufferDescriptor *m_SGTable = nullptr;
