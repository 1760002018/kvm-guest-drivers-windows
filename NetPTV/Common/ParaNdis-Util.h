...
#ifdef DBG
void NetPtvAssert(bool Statement, ULONG Code);
#define NETPTV_ASSERT(x) NetPtvAssert((x) ? true : false, __LINE__)
#else
#define NETPTV_ASSERT(x) for(;;) break
#endif
...
public:
    ...
    _Ndis_acquires_exclusive_lock_(this->m_Lock)
        void LockDPR()
    {
        NETPTV_ASSERT(NDIS_CURRENT_IRQL() == DISPATCH_LEVEL);
...
LONG FORCEINLINE NetPtvInterlockedAdd(
...
public:
    ...
    LONG AddRef(ULONG RefCnt) { return NetPtvInterlockedAdd(&m_Counter, (LONG)RefCnt); }
    LONG Release() { return NdisInterlockedDecrement(&m_Counter); }
    LONG Release(ULONG RefCnt) { return NetPtvInterlockedAdd(&m_Counter, -(LONG)RefCnt); }
