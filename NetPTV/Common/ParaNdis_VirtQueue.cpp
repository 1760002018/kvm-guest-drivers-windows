...
bool CVirtQueue::AllocateQueueMemory()
{
    ...
    {
        DPrintf(0, "[%s] phyzio_query_queue_allocation(%d) failed with error %x\n", __FUNCTION__, m_Index, status);
        return false;
    }
...
void CVirtQueue::Renew()
{
    ...
    NTSTATUS status = phyzio_find_queue(
        m_IODevice,
        m_Index,
        &m_VirtQueue);
    ...
    NDIS_HANDLE DrvHandle)
{
    ...
    NETPTV_ASSERT(m_VirtQueue == nullptr);
...
void CVirtQueue::Delete()
{
    ...
    {
        phyzio_delete_queue(m_VirtQueue);
        m_VirtQueue = nullptr;
    }
}
...
{
    return phyzio_set_queue_vector(m_VirtQueue, vector);
}
    ...
    PPARANDIS_ADAPTER Context)
{
    ...
    m_SGTableCapacity = m_Context->bUseIndirect ? phyzio_get_indirect_page_capacity() : GetRingSize();

    auto SGBuffer = ParaNdis_AllocateMemoryRaw(m_DrvHandle, m_SGTableCapacity * sizeof(m_SGTable[0]));
    m_SGTable = static_cast<struct PhyzIOBufferDescriptor *>(SGBuffer);
...
{
    m_UsedBuffersNum = m_Indirect ? 1 : m_CurrPhyzioSGLEntry;
    ...
    if (0 <= Queue->AddBuf(m_PhyzioSGL,
                           m_CurrPhyzioSGLEntry,
                           0,
                           this,
...
{
    if (m_CurrPhyzioSGLEntry < m_PhyzioSGLSize)
    {
        m_PhyzioSGL[m_CurrPhyzioSGLEntry].physAddr = PA;
        m_PhyzioSGL[m_CurrPhyzioSGLEntry].length = Length;
        m_CurrPhyzioSGLEntry++;
...
{
    m_CurrPhyzioSGLEntry = 0;

    if (m_Headers.VlanHeader()->TCI == 0)
    {
        if (m_AnyLayout)
        {
            return AddDataChunk(m_Headers.PhyzioHeaderPA(), m_Headers.PhyzioHeaderLength() +
                                ParsedHeadersLength);
        }
        else
        {
    ...
    {
        NETPTV_ASSERT(ParsedHeadersLength >= ETH_HEADER_SIZE);

        if (!AddDataChunk(m_Headers.PhyzioHeaderPA(), m_Headers.PhyzioHeaderLength()) ||
...
bool CTXHeaders::Allocate()
{
    if (m_HeadersBuffer.Allocate(PAGE_SIZE))
    {
        ...
        m_VlanHeaderVA   = VA;
        m_PhyzioHeaderVA = RtlOffsetToPointer(m_VlanHeaderVA, prioSize);
        m_EthHeaderVA    = RtlOffsetToPointer(m_PhyzioHeaderVA, m_PhyzioHdrSize);
        m_IPHeadersVA    = RtlOffsetToPointer(m_EthHeaderVA, ETH_HEADER_SIZE);

        m_PhyzioHeaderPA.QuadPart = PA.QuadPart + RtlPointerToOffset(VA, m_PhyzioHeaderVA);
        ...
        m_MaxEthHeadersSize = m_HeadersBuffer.GetSize() - prioSize - m_PhyzioHdrSize;
