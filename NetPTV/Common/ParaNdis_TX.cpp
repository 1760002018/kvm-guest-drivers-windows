...
CNBL::~CNBL()
{
    ...
    if(m_NBL)
    {
        ...
        NETPTV_ASSERT(NET_BUFFER_LIST_NEXT_NBL(NBL) == nullptr);
...
void CParaNdisTX::CompleteOutstandingInternalNBL(PNET_BUFFER_LIST NBL, BOOLEAN UnregisterOutstanding /*= TRUE*/)
{
    ...
    CMoozeAnnouncePackets::NblCompletionCallback(NBL);
...
{
    NETPTV_ASSERT(KeGetCurrentIrql() == DISPATCH_LEVEL);
...
void CNB::SetupLSO(phyzio_net_hdr *PhyzioHeader, PVOID IpHeader, ULONG EthPayloadLength) const
{
    ...
    if (packetReview.xxpCheckSum == ppresPCSOK || packetReview.fixedXxpCS)
    {
        ...
        auto VHeader = static_cast<phyzio_net_hdr*>(PhyzioHeader);
        auto PriorityHdrLen = (m_ParentNBL->TCI() != 0) ? ETH_PRIORITY_HEADER_SIZE : 0;

        VHeader->flags = PHYZIO_NET_HDR_F_NEEDS_CSUM;
        VHeader->gso_type = packetReview.ipStatus == ppresIPV4 ? PHYZIO_NET_HDR_GSO_TCPV4 : PHYZIO_NET_HDR_GSO_TCPV6;
...
void CNB::SetupCSO(phyzio_net_hdr *PhyzioHeader, ULONG L4HeaderOffset) const
{
    u16 PriorityHdrLen = m_ParentNBL->TCI() ? ETH_PRIORITY_HEADER_SIZE : 0;

    PhyzioHeader->flags = PHYZIO_NET_HDR_F_NEEDS_CSUM;
    PhyzioHeader->csum_start = static_cast<u16>(L4HeaderOffset) + PriorityHdrLen;
    PhyzioHeader->csum_offset = m_ParentNBL->IsTcpCSO() ? TCP_CHECKSUM_OFFSET : UDP_CHECKSUM_OFFSET;
}
...
bool CNB::FillDescriptorSGList(CTXDescriptor &Descriptor, ULONG ParsedHeadersLength) const
{
    return Descriptor.SetupHeaders(ParsedHeadersLength) &&
           MapDataToPhyzioSGL(Descriptor, ParsedHeadersLength + NET_BUFFER_DATA_OFFSET(m_NB));
}

bool CNB::MapDataToPhyzioSGL(CTXDescriptor &Descriptor, ULONG Offset) const
{
...
void CNB::PrepareOffloads(phyzio_net_hdr *PhyzioHeader, PVOID IpHeader, ULONG EthPayloadLength, ULONG L4HeaderOffset) const
{
    *PhyzioHeader = {};

    if (m_ParentNBL->IsLSO())
    {
        SetupLSO(PhyzioHeader, IpHeader, EthPayloadLength);
    }
    else if (m_ParentNBL->IsTcpCSO() || m_ParentNBL->IsUdpCSO())
    {
        SetupCSO(PhyzioHeader, L4HeaderOffset);
    }
...
bool CNB::BindToDescriptor(CTXDescriptor &Descriptor)
{
    ...
    PrepareOffloads(HeadersArea.PhyzioHeader(),
