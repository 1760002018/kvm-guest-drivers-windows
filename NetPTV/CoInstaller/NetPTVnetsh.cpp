#include "stdafx.h"
#include "NetPTVnetsh.h"
#include "RegAccess.h"
#include "NetPTVAux.h"
#include "RegParam.h"

//This is NetSH Helper GUID {D9C599C4-8DCF-4a6a-93AA-A16FE6D5125C}
static const GUID NETPTV_HELPER_GUID =
    { 0xd9c599c4, 0x8dcf, 0x4a6a, { 0x93, 0xaa, 0xa1, 0x6f, 0xe6, 0xd5, 0x12, 0x5c } };
static const DWORD NETPTV_HELPER_VERSION = 1;

static const LPCTSTR NETPTV_HELPER_NAME = TEXT("NetPTV");
static const LPWSTR NETPTV_HELPER_NAME_W = L"NetPTV";

static const LPCTSTR NETPTV_DEVICES_CLASS = TEXT("net");

//TODO: put real device numbers here
static const LPCTSTR NETPTV_DEVICE_VENDOR_ID = TEXT("");
static const LPCTSTR NETPTV_DEVICE_HARDWARE_ID = TEXT("");

static const LPCWSTR NETPTV_IDX_PARAM_NAME   = L"idx";
static const LPCTSTR NETPTV_IDX_PARAM_NAME_T   = TEXT("idx");
static const LPCWSTR NETPTV_PARAM_PARAM_NAME = L"param";
static const LPCTSTR NETPTV_PARAM_PARAM_NAME_T = TEXT("param");
static const LPCWSTR NETPTV_VALUE_PARAM_NAME = L"value";
static const LPCTSTR NETPTV_VALUE_PARAM_NAME_T = TEXT("value");

static HINSTANCE g_hinstThisDLL = NULL;

static bool _NetPTVGetDeviceClassGuids(vector<GUID>& GUIDs)
{
    GUID *pguidDevClassPtr = NULL;
    DWORD dwNumGuids;

    if(!SetupDiClassGuidsFromNameEx(NETPTV_DEVICES_CLASS, NULL, 0,
...
#pragma warning(suppress: 6102)
        ...
        if(!SetupDiClassGuidsFromNameEx(NETPTV_DEVICES_CLASS, pguidDevClassPtr, dwNumGuids,
                                        &dwNumGuids, NULL, NULL))
        {
...
tstring _NetPTVQueryDeviceString(HDEVINFO hDeviceSet, PSP_DEVINFO_DATA DeviceInfoData, DWORD dwPropertyID)
{
...
DWORD _NetPTVQueryDeviceDWORD(HDEVINFO hDeviceSet, PSP_DEVINFO_DATA DeviceInfoData, DWORD dwPropertyID)
{
    DWORD dwDeviceDword;
    DWORD dwDataType;
...
static HKEY _NetPTVOpenDeviceSwRegKey(HDEVINFO hDeviceSet, PSP_DEVINFO_DATA DeviceInfoData)
{
...
typedef struct tag_NetPTVDeviceInfo
{
    tstring strDeviceID;
...
} _NetPTVDeviceInfo, *_PNetPTVDeviceInfo;

bool _NetPTVIsKnownDevice(const tstring &strDeviceID)
{
    return (tstring::npos != strDeviceID.find(tstring(TEXT("VEN_")) + NETPTV_DEVICE_VENDOR_ID)) &&
           (tstring::npos != strDeviceID.find(tstring(TEXT("DEV_")) + NETPTV_DEVICE_HARDWARE_ID));
}

tstring _NetPTVKeyPathFromDeviceSwHKEY(HKEY hKey)
{
    tstring strNativePath = wstring2tstring(NetPTVGetKeyPathFromKKEY(hKey));
...
static bool _NetPTVGetKnownDevices(GUID *pguidDevClassPtr, vector<_NetPTVDeviceInfo>& Devices, HDEVINFO *phDeviceInfo)
{
    *phDeviceInfo = SetupDiGetClassDevsEx(pguidDevClassPtr, NULL, NULL, DIGCF_PRESENT, NULL, NULL, NULL);
    ...
    for(DWORD dwDevIndex = 0; SetupDiEnumDeviceInfo(*phDeviceInfo, dwDevIndex, &CurrDeviceInfo); dwDevIndex++)
    {
        ...
        if(CR_SUCCESS == err)
        {
            if(_NetPTVIsKnownDevice(szDeviceId))
            {
                _NetPTVDeviceInfo ResDevInfo;
                ResDevInfo.strDeviceID           = szDeviceId;
                ResDevInfo.strDeviceDescription  = _NetPTVQueryDeviceString(*phDeviceInfo, &CurrDeviceInfo, SPDRP_DEVICEDESC);
                ResDevInfo.strDeviceFriendlyName = _NetPTVQueryDeviceString(*phDeviceInfo, &CurrDeviceInfo, SPDRP_FRIENDLYNAME);
                if (ResDevInfo.strDeviceFriendlyName.length() == 0)
                {
                    ResDevInfo.strDeviceFriendlyName = ResDevInfo.strDeviceDescription;
                }
                ResDevInfo.strLocationInfo = _NetPTVQueryDeviceString(*phDeviceInfo, &CurrDeviceInfo, SPDRP_LOCATION_INFORMATION);
                ResDevInfo.dwDeviceNumber = _NetPTVQueryDeviceDWORD(*phDeviceInfo, &CurrDeviceInfo, SPDRP_UI_NUMBER);
                ResDevInfo.DevInfoData = CurrDeviceInfo;
                HKEY hSoftwareRegKey = _NetPTVOpenDeviceSwRegKey(*phDeviceInfo, &CurrDeviceInfo);
                if(INVALID_HANDLE_VALUE != hSoftwareRegKey)
                {
                    ResDevInfo.strRegPathName = _NetPTVKeyPathFromDeviceSwHKEY(hSoftwareRegKey);
...
static pair< HDEVINFO, vector<_NetPTVDeviceInfo> > _NetPTVGetDevicesOfInterest()
{
    vector<GUID> GUIDs;
    pair< HDEVINFO, vector<_NetPTVDeviceInfo> > Devices;

    _NetPTVGetDeviceClassGuids(GUIDs);
    if(GUIDs.empty())
        return Devices;

    _NetPTVGetKnownDevices(&GUIDs[0], Devices.second, &Devices.first);
    return Devices;
}

void _NetPTVDumpDeviceInfo(DWORD dwIndex, const _NetPTVDeviceInfo &DeviceInfo)
{
...
static vector<_NetPTVDeviceInfo> g_DevicesOfInterest;

static DWORD _NetPTVRestartDevice(DWORD dwIndex)
{
...
static DWORD __NetPTVwsz2DWORD(PWSTR wszInput)
{
...
static BOOL __NetPTVConvertDeviceIndex(PWSTR wszIndex, PDWORD pdwIndex)
{
    *pdwIndex = __NetPTVwsz2DWORD(wszIndex);
...
static void _NetPTVFillDeviceParamsList(DWORD dwDeviceIndex, ParamDescrListT& List)
{
    auto_ptr<neTPTVRegParam> pParam;
    DWORD dwParamIndex = 0;
    neTPTVRegAccess DeviceRegKey(HKEY_LOCAL_MACHINE, g_DevicesOfInterest[dwDeviceIndex].strRegPathName.c_str());

    while(NULL != (pParam = auto_ptr<neTPTVRegParam>(neTPTVRegParam::GetParam(DeviceRegKey, dwParamIndex++))).get())
    {
...
static void _NetPTVFillDeviceValuesList(DWORD dwDeviceIndex, ParamValueListT& List)
{
    auto_ptr<neTPTVRegParam> pParam;
    DWORD dwParamIndex = 0;
    neTPTVRegAccess DeviceRegKey(HKEY_LOCAL_MACHINE, g_DevicesOfInterest[dwDeviceIndex].strRegPathName.c_str());
...
static bool _NetPTVQueryDetailedParamInfo(DWORD dwDeviceIndex,
                                          const tstring &strParamName,
                                          neTPTVRegParamType &ParamType,
                                          neTPTVRegParamExInfoList &ParamInfo)
{
    neTPTVRegAccess DeviceRegKey(HKEY_LOCAL_MACHINE, g_DevicesOfInterest[dwDeviceIndex].strRegPathName.c_str());

    auto_ptr<neTPTVRegParam> pParam(neTPTVRegParam::GetParam(DeviceRegKey, strParamName.c_str()));

    ParamType = neTPTVRegParam::GetType(DeviceRegKey, strParamName.c_str());
    if(NETPTV_RTT_UNKNOWN == ParamType)
    {
        return false;
    }
...
static bool _NetPTVQueryParamValue(DWORD dwDeviceIndex,
                                   const tstring &strParamName,
...
static bool _NetPTVSetParamValue(DWORD dwDeviceIndex,
                                 const tstring &strParamName,
...
// Remarks:
//
//      Lists all NetPTV devices currently present in the system.
//      Each device is assigned unique index.
//      The index is used to identify the device for all other NetPTV commands.
//
DWORD WINAPI _NetPTVShowDevicesCmdHandler(__in   PWCHAR  /*pwszMachine*/,
                                          ...
                                          __out  BOOL*   pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        NETCO_DEBUG_PRINT(TEXT("_NetPTVShowDevicesCmdHandler called"));
        int i = 0;
        for(vector<_NetPTVDeviceInfo>::const_iterator it = g_DevicesOfInterest.begin();
            it != g_DevicesOfInterest.end();
            ++it, ++i)
        {
            _NetPTVDumpDeviceInfo(i, *it);
        }
        return NO_ERROR;
    }
...
//
DWORD WINAPI _NetPTVShowParamInfoCmdHandler (__in   PWCHAR  /*pwszMachine*/,
                                             __in   PWCHAR* ppwcArguments,
                                             __in   DWORD   dwCurrentIndex,
                                             __in   DWORD   dwArgCount,
                                             __in   DWORD   /*dwFlags*/,
                                             ...
                                             __out  BOOL*   pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        NETCO_DEBUG_PRINT(TEXT("_NetPTVShowParamInfoCmdHandler called"));
        TAG_TYPE TagsList[] =
            { {NETPTV_IDX_PARAM_NAME,   NS_REQ_PRESENT},
              {NETPTV_PARAM_PARAM_NAME, NS_REQ_PRESENT} };
        ...
        case NO_ERROR:
            {
                DWORD dwIndex;
                if(__NetPTVConvertDeviceIndex(ppwcArguments[dwCurrentIndex + pdwTagMatchResults.get()[0]], &dwIndex))
                {
                    wstring wstrParamName = ppwcArguments[dwCurrentIndex + pdwTagMatchResults.get()[1]];
                    neTPTVRegParamExInfoList ParamInfo;
                    neTPTVRegParamType ParamType;

                    if(!_NetPTVQueryDetailedParamInfo(dwIndex, wstring2tstring(wstrParamName),ParamType, ParamInfo))
                    {
...
//
DWORD WINAPI _NetPTVGetParamCmdHandler (__in   PWCHAR  /*pwszMachine*/,
                                        __in   PWCHAR* ppwcArguments,
                                        __in   DWORD   dwCurrentIndex,
                                        __in   DWORD   dwArgCount,
                                        __in   DWORD   /*dwFlags*/,
                                        ...
                                        __out  BOOL*   pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        NETCO_DEBUG_PRINT(TEXT("_NetPTVGetParamCmdHandler called"));
        TAG_TYPE TagsList[] =
            { {NETPTV_IDX_PARAM_NAME,   NS_REQ_PRESENT},
              {NETPTV_PARAM_PARAM_NAME, NS_REQ_PRESENT} };
        ...
        case NO_ERROR:
            {
                ...
                if(__NetPTVConvertDeviceIndex(ppwcArguments[dwCurrentIndex + pdwTagMatchResults.get()[0]], &dwIndex))
                {
                    ...
                    if(!_NetPTVQueryParamValue(dwIndex, strParamName, strParamValue))
                    {
                        return ERROR_INVALID_PARAMETER;
                    }
...
//
DWORD WINAPI _NetPTVSetParamCmdHandler (__in   PWCHAR  /*pwszMachine*/,
                                        __in   PWCHAR* ppwcArguments,
                                        __in   DWORD   dwCurrentIndex,
                                        __in   DWORD   dwArgCount,
                                        __in   DWORD   /*dwFlags*/,
                                        ...
                                        __out  BOOL*   pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        NETCO_DEBUG_PRINT(TEXT("_NetPTVSetParamCmdHandler called"));
        TAG_TYPE TagsList[] =
            { {NETPTV_IDX_PARAM_NAME,   NS_REQ_PRESENT},
              {NETPTV_PARAM_PARAM_NAME, NS_REQ_PRESENT},
              {NETPTV_VALUE_PARAM_NAME, NS_REQ_PRESENT} };
        ...
        case NO_ERROR:
            {
                ...
                if(__NetPTVConvertDeviceIndex(ppwcArguments[dwCurrentIndex + pdwTagMatchResults.get()[0]], &dwIndex))
                {
                    ...
                    if(!_NetPTVSetParamValue(dwIndex, wstring2tstring(wstrParamName), wstring2tstring(wstrParamValue)))
                    {
                        return ERROR_INVALID_PARAMETER;
                    }
                    return NO_ERROR;
...
//      show parameters 2
//
DWORD WINAPI _NetPTVShowParamsCmdHandler(__in   PWCHAR  /*pwszMachine*/,
                                         __in   PWCHAR* ppwcArguments,
                                         __in   DWORD   dwCurrentIndex,
                                         __in   DWORD   dwArgCount,
                                         __in   DWORD   /*dwFlags*/,
                                         ...
                                         __out  BOOL*   pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        NETCO_DEBUG_PRINT(TEXT("_NetPTVShowParamsCmdHandler called"));
        TAG_TYPE TagsList[] =
            { {NETPTV_IDX_PARAM_NAME,   NS_REQ_PRESENT} };
        ...
        case NO_ERROR:
            {
                ...
                if(__NetPTVConvertDeviceIndex(ppwcArguments[dwCurrentIndex + pdwTagMatchResults.get()[0]], &dwIndex))
                {
                    ...
                    _NetPTVFillDeviceParamsList(dwIndex, Params);
...
//      restart 2
//
DWORD WINAPI _NetPTVRestartDeviceCmdHandler(__in   PWCHAR  /*pwszMachine*/,
                                            __in   PWCHAR* ppwcArguments,
                                            __in   DWORD   dwCurrentIndex,
                                            __in   DWORD   dwArgCount,
                                            __in   DWORD   /*dwFlags*/,
                                            ...
                                            __out  BOOL*   pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        NETCO_DEBUG_PRINT(TEXT("_NetPTVRestartDeviceCmdHandler called"));
        TAG_TYPE TagsList[] =
            { {NETPTV_IDX_PARAM_NAME,   NS_REQ_PRESENT} };
        ...
        case NO_ERROR:
            {
                ...
                if(__NetPTVConvertDeviceIndex(ppwcArguments[dwCurrentIndex + pdwTagMatchResults.get()[0]], &dwIndex))
                {
                    ...
                    DWORD dwError = _NetPTVRestartDevice(dwIndex);
                    switch(dwError)
                    {
                    case NO_ERROR:
...
#define CMD_NETPTV_SHOW_DEVICES       L"devices"
#define HLP_NETPTV_SHOW_DEVICES       IDS_SHOWDEVICESSHORT
#define HLP_NETPTV_SHOW_DEVICES_EX    IDS_SHOWDEVICESLONG
#define CMD_NETPTV_SHOW_PARAMS        L"parameters"
#define HLP_NETPTV_SHOW_PARAMS        IDS_SHOWPARAMSSHORT
#define HLP_NETPTV_SHOW_PARAMS_EX     IDS_SHOWPARAMSLONG
#define CMD_NETPTV_SHOW_PARAMINFO     L"paraminfo"
#define HLP_NETPTV_SHOW_PARAMINFO     IDS_SHOWPARAMINFOSHORT
#define HLP_NETPTV_SHOW_PARAMINFO_EX  IDS_SHOWPARAMINFOLONG

CMD_ENTRY  g_ShowCmdTable[] =
{
    CREATE_CMD_ENTRY_EX(NETPTV_SHOW_DEVICES,
                        (PFN_HANDLE_CMD) _NetPTVShowDevicesCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
    CREATE_CMD_ENTRY_EX(NETPTV_SHOW_PARAMS,
                        (PFN_HANDLE_CMD) _NetPTVShowParamsCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
    CREATE_CMD_ENTRY_EX(NETPTV_SHOW_PARAMINFO,
                        (PFN_HANDLE_CMD) _NetPTVShowParamInfoCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL)

};

#define CMD_NETPTV_RESTART_DEVICE       L"restart"
#define CMD_NETPTV_RESTART_DEVICE_T     TEXT("restart")
#define HLP_NETPTV_RESTART_DEVICE       IDS_RESTARTDEVICE
#define HLP_NETPTV_RESTART_DEVICE_EX    IDS_RESTARTDEVICELONG
#define CMD_NETPTV_GET_PARAM            L"getparam"
#define HLP_NETPTV_GET_PARAM            IDS_GETPARAM
#define HLP_NETPTV_GET_PARAM_EX         IDS_GETPARAMLONG
#define CMD_NETPTV_SET_PARAM            L"setparam"
#define CMD_NETPTV_SET_PARAM_T          TEXT("setparam")
#define HLP_NETPTV_SET_PARAM            IDS_SETPARAM
#define HLP_NETPTV_SET_PARAM_EX         IDS_SETPARAMLONG

CMD_ENTRY  g_TopLevelCommands[] =
{
    CREATE_CMD_ENTRY_EX(NETPTV_RESTART_DEVICE,
                        (PFN_HANDLE_CMD) _NetPTVRestartDeviceCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
    CREATE_CMD_ENTRY_EX(NETPTV_GET_PARAM,
                        (PFN_HANDLE_CMD) _NetPTVGetParamCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
    CREATE_CMD_ENTRY_EX(NETPTV_SET_PARAM,
                        (PFN_HANDLE_CMD) _NetPTVSetParamCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL)
};
...
DWORD WINAPI _NetPTVDumpCdmHandler(__in  PWCHAR      pwszRouter,
                                   ...
                                   __in  PVOID    /* pvData */)
{
    try
    {
        NETCO_DEBUG_PRINT(TEXT("_NetPTVDumpCdmHandler called"));
        ...
        tcout << endl<< TEXT("pushd ") << NETPTV_HELPER_NAME << endl << endl;

        for(DWORD i = 0; i < g_DevicesOfInterest.size(); i++)
        {
            ParamValueListT ValuesList;
            _NetPTVFillDeviceValuesList(i, ValuesList);

            for(ParamValueListT::const_iterator it = ValuesList.begin();
                it != ValuesList.end(); ++it)
            {
                tcout << CMD_NETPTV_SET_PARAM_T << TEXT(" ")
                      << NETPTV_IDX_PARAM_NAME_T << TEXT("=") << i << TEXT(" ")
                      << NETPTV_PARAM_PARAM_NAME_T << TEXT("=") << it->first << TEXT(" ")
                      << NETPTV_VALUE_PARAM_NAME_T << TEXT("=") << it->second << endl;
            }

            tcout << CMD_NETPTV_RESTART_DEVICE_T << TEXT(" ")
                  << NETPTV_IDX_PARAM_NAME_T << TEXT("=") << i << TEXT(" ")
                  << endl << endl;
        }
...
DWORD WINAPI _NetPTVNetshStartHelper(__in  const GUID *pguidParent,
                                     __in  DWORD dwVersion)
{
    try
    {
        ...
        NETCO_DEBUG_PRINT(TEXT("_NetPTVNetshStartHelper called"));

        pair< HDEVINFO, vector<_NetPTVDeviceInfo> > Devices = _NetPTVGetDevicesOfInterest();
        ...
        attr.dwVersion = NETPTV_HELPER_VERSION;
        attr.dwReserved = 0;
        attr.pwszContext = NETPTV_HELPER_NAME_W;
        attr.guidHelper = NETPTV_HELPER_GUID;
        ...
        attr.pfnDumpFn = (PNS_CONTEXT_DUMP_FN) _NetPTVDumpCdmHandler;
...
DWORD WINAPI _NetPTVNetshStopHelper(__in  DWORD dwReserved)
{
    UNREFERENCED_PARAMETER(dwReserved);

    NETCO_DEBUG_PRINT(TEXT("_NetPTVNetshStopHelper called"));
                                   ...
                                   PVOID pReserved)
{
    ...
    attr.guidHelper = NETPTV_HELPER_GUID;
    attr.dwVersion  = NETPTV_HELPER_VERSION;
    attr.pfnStart   = _NetPTVNetshStartHelper;
    attr.pfnStop    = _NetPTVNetshStopHelper;
...
DWORD NETCO_API RegisterNetPTVNetShHelper(void)
{
    try
    {
        ...
        neTPTVRegAccess regAccess(NETSH_HELPERS_LIST_HIVE, NETSH_HELPERS_LIST_PATH);
        if(!regAccess.WriteString(NETPTV_HELPER_NAME, szDllPathName))
        {
            dwErr = GetLastError();
            tstringstream strmError;
...
DWORD NETCO_API UnregisterNetPTVNetShHelper(void)
{
    try
    {
        neTPTVRegAccess regAccess(NETSH_HELPERS_LIST_HIVE, NETSH_HELPERS_LIST_PATH);
        regAccess.DeleteValue(NETPTV_HELPER_NAME);
        return ERROR_SUCCESS;
    }
    catch (const exception& ex)
    {
