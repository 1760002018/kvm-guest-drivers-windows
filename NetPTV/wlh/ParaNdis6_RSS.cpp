...
static VOID InitRSSCapabilities(PARANDIS_ADAPTER *pContext)
{
    ...
    {
        ULONG flags = 0;
        flags |= (pContext->DeviceRSSCapabilities.SupportedHashes & PHYZIO_NET_RSS_HASH_TYPE_TCPv4) ?
            NDIS_RSS_CAPS_HASH_TYPE_TCP_IPV4 : 0;
        flags |= (pContext->DeviceRSSCapabilities.SupportedHashes & PHYZIO_NET_RSS_HASH_TYPE_TCPv6) ?
            NDIS_RSS_CAPS_HASH_TYPE_TCP_IPV6 : 0;
        flags |= (pContext->DeviceRSSCapabilities.SupportedHashes & PHYZIO_NET_RSS_HASH_TYPE_TCP_EX) ?
    ...
    if (CheckNdisVersion(6, 80))
    {
        ...
        {
            ULONG flags = 0;
            flags |= (pContext->DeviceRSSCapabilities.SupportedHashes & PHYZIO_NET_RSS_HASH_TYPE_UDPv4) ?
                NDIS_RSS_CAPS_HASH_TYPE_UDP_IPV4 : 0;
            flags |= (pContext->DeviceRSSCapabilities.SupportedHashes & PHYZIO_NET_RSS_HASH_TYPE_UDPv6) ?
                NDIS_RSS_CAPS_HASH_TYPE_UDP_IPV6 : 0;
            flags |= (pContext->DeviceRSSCapabilities.SupportedHashes & PHYZIO_NET_RSS_HASH_TYPE_UDP_EX) ?
...
void ParaNdis6_CheckDeviceRSSCapabilities(PARANDIS_ADAPTER *pContext, bool& bRss, bool& bHash)
{
    ...
    BOOLEAN bResult = (pContext->DeviceRSSCapabilities.SupportedHashes & PHYZIO_NET_RSS_HASH_TYPE_IPv4) &&
...
{
    ULONG val = 0;
    val |= (hashSettings & NDIS_HASH_IPV4) ? PHYZIO_NET_RSS_HASH_TYPE_IPv4 : 0;
    val |= (hashSettings & NDIS_HASH_TCP_IPV4) ? PHYZIO_NET_RSS_HASH_TYPE_TCPv4 : 0;
    val |= (hashSettings & NDIS_HASH_IPV6) ? PHYZIO_NET_RSS_HASH_TYPE_IPv6 : 0;
    val |= (hashSettings & NDIS_HASH_IPV6_EX) ? PHYZIO_NET_RSS_HASH_TYPE_IP_EX : 0;
    val |= (hashSettings & NDIS_HASH_TCP_IPV6) ? PHYZIO_NET_RSS_HASH_TYPE_TCPv6 : 0;
    val |= (hashSettings & NDIS_HASH_TCP_IPV6_EX) ? PHYZIO_NET_RSS_HASH_TYPE_TCP_EX : 0;
#if (NDIS_SUPPORT_NDIS680)
    val |= (hashSettings & NDIS_HASH_UDP_IPV4) ? PHYZIO_NET_RSS_HASH_TYPE_UDPv4 : 0;
    val |= (hashSettings & NDIS_HASH_UDP_IPV6) ? PHYZIO_NET_RSS_HASH_TYPE_UDPv6 : 0;
    val |= (hashSettings & NDIS_HASH_UDP_IPV6_EX) ? PHYZIO_NET_RSS_HASH_TYPE_UDP_EX : 0;
#endif
...
static void SetDeviceRSSSettings(PARANDIS_ADAPTER *pContext, bool bForceOff = false)
{
    ...
    UCHAR command = pContext->bRSSSupportedByDevice ?
        PHYZIO_NET_CTRL_MQ_RSS_CONFIG : PHYZIO_NET_CTRL_MQ_HASH_CONFIG;

    if (pContext->RSSParameters.RSSMode == PARANDIS_RSS_DISABLED || bForceOff)
    {
        phyzio_net_rss_config cfg = {};
        cfg.max_tx_vq = (USHORT)pContext->nPathBundles;
        pContext->CXPath.SendControlMessage(PHYZIO_NET_CTRL_MQ, command, &cfg, sizeof(cfg), NULL, 0, 2);
    }
    else
    {
        phyzio_net_rss_config *cfg;
        ...
        config_size = phyzio_net_rss_config_size(indirection_table_len, hash_key_len);
        cfg = (phyzio_net_rss_config *)ParaNdis_AllocateMemory(pContext, config_size);
        if (!cfg)
        {
            return;
        }
        ...
        pContext->CXPath.SendControlMessage(PHYZIO_NET_CTRL_MQ, command, cfg, config_size, NULL, 0, 2);
