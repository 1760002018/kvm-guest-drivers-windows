...
)
{
    ...
    default:
        PioGpuDbgBreak();
        PIOGPU_LOG_ASSERTION1("Invalid rotation (0x%I64x) specified", pBltInfo->Rotation);
        *pPixelPitch = 0;
        *pRowPitch = 0;
        return;
    }
    ...
    _In_reads_(NumRects) CONST RECT *pRects)
{
    ...
    for (UINT iRect = 0; iRect < NumRects; iRect++) {
        ...
        PIOGPU_ASSERT(pRect->right >= pRect->left);
        PIOGPU_ASSERT(pRect->bottom >= pRect->top);
...
VOID CopyBits32_32(
    ...
    _In_reads_(NumRects) CONST RECT *pRects)
{
    PIOGPU_ASSERT((pDst->BitsPerPel == 32) &&
        (pSrc->BitsPerPel == 32));
    PIOGPU_ASSERT((pDst->Rotation == D3DKMDT_VPPR_IDENTITY) &&
...
UINT BPPFromPixelFormat(D3DDDIFORMAT Format)
{
    ...
    default: PIOGPU_LOG_ASSERTION1("Unknown D3DDDIFORMAT 0x%I64x", Format); return 0;
    }
}
