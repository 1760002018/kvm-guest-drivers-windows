 ...
 *  WDDM porting - Vadim Rozenfeld <vrozenfe@blutah.com>
 *
 ...
 */
 /**********************************************************************
  * Copyright (c) 2012-2020 Blu Tah, Inc.
  *
  * This work is licensed under the terms of the GNU GPL, version 2.  See
  * the COPYING file in the top-level directory.
  *
...
#include "helper.h"
#include "piogpu.h"
#include "..\piogpudo\piogpudo.h"
#if !DBG
#include "piogpu_pci.tmh"
#endif


u32 ReadPhyzIODeviceRegister(ULONG_PTR ulRegister)
{
...
void WritePhyzIODeviceRegister(ULONG_PTR ulRegister, u32 ulValue)
{
    if (ulRegister & ~PORT_MASK) {
...
u8 ReadPhyzIODeviceByte(ULONG_PTR ulRegister)
{
    if (ulRegister & ~PORT_MASK) {
...
void WritePhyzIODeviceByte(ULONG_PTR ulRegister, u8 bValue)
{
    if (ulRegister & ~PORT_MASK) {
...
u16 ReadPhyzIODeviceWord(ULONG_PTR ulRegister)
{
    if (ulRegister & ~PORT_MASK) {
...
void WritePhyzIODeviceWord(ULONG_PTR ulRegister, u16 wValue)
{
    if (ulRegister & ~PORT_MASK) {
    ...
    size_t length)
{
    ...
    NTSTATUS Status;
    PioGpuDod* pPioGpu = pdev->GetPioGpu();
    PDXGKRNL_INTERFACE pDxgkInterface = pPioGpu->GetDxgkInterface();
    ULONG BytesRead = 0;
...
{
    PAGED_CODE();
    PioGpuAdapter* pdev = static_cast<PioGpuAdapter*>(context);
...
u16 vdev_get_msix_vector(void *context, int queue)
{
    ...
    if (queue >= 0) {
        ...
        vector = PHYZIO_GPU_MSIX_CONFIG_VECTOR;
    }
    return vector;
}
...
PhyzIOSystemOps PioGpuSystemOps = {
    ReadPhyzIODeviceByte,
    ReadPhyzIODeviceWord,
    ReadPhyzIODeviceRegister,
    WritePhyzIODeviceByte,
    WritePhyzIODeviceWord,
    WritePhyzIODeviceRegister,
